...
namespace FLive.Web.Controllers.API
{
    ...
    [Authorize]
    public class LiveWorkoutController : Controller
    {
        ...
        public LiveWorkoutController(ILiveWorkoutService liveWorkoutService, IFileUploadRepository fileUploadRepository , ISubscriberService subscriberService, IConnectionManager connectionManager)
        {
        ...
        [HttpPost("{liveworkoutId}/instructions")]
        [Produces("application/json")]
        [Consumes("multipart/form-data", "application/json-patch+json", "application/json")]
        public async Task<LiveWorkout> UploadInstructionsFile(long liveworkoutId, IFormFile file)
        {
            var uploadedUrl = "";

            var stream = file.OpenReadStream();
            uploadedUrl = await _fileUploadRepository.UploadFileAsBlob(stream, file.FileName, "instructions");
            await _liveWorkoutService.SetInstructionsFile(liveworkoutId, uploadedUrl);
            return await _liveWorkoutService.Get(liveworkoutId);
